...
interface IChainLink{
    ...
    function latestRound() external returns(
        uint256 roundId
    );

    // Historical Data-Feed
    function getAnswer(uint256 _roundId) external returns(
        int256 answer
    );
    function getRoundData(uint80 _roundId) external returns(
        uint80 roundId,
        int256 answer,
        uint256 startedAt,
        uint256 updatedAt,
    ...
    function getTimestamp(uint256 _roundId) external returns(uint256 updatedAt);
}
...
contract ChainSeed is AccessControl, ReentrancyGuard {
    ...
    // Developer
    function addTrustedSource(address[] memory _provider) public onlyRole(DEV_ROLE) nonReentrant() {
        require(_provider.length > 0, "SEED: No provider provided");
        for(uint256 i = 0; i < _provider.length; i++){
            if(_provider[i] == address(0)) revert("SEED: Provider address must not be zero");
            if(_provider[i] == address(this)) revert("SEED: Provider address is not valid");
            if(_dataFeeds[_provider[i]]) revert("SEED: Provider is already listed");

            _feedCount++;
            _dataFeeds[_provider[i]] = true;

            if(!_isProviderExists(_provider[i]))
                _providers.push(_provider[i]);
        }
    }
    function removeTrustedSource(address[] memory _provider) public onlyRole(DEV_ROLE) nonReentrant() {
        for(uint256 i = 0; i < _provider.length; i++){
    ...
    function grantWhitelist(address _owner, bool _active) public onlyRole(DEV_ROLE){
        require(_owner != address(this) && _owner != address(0), "WHITELIST: Invalid owner");
    ...
    // Public
    function randomSeed() public onlyWhitelist() nonReentrant() returns(bytes32 seed) {
    ...
    function _secureRandomSeed() private returns(bytes32 seed) {
        ...
        uint256 maxLoopCount = 10;
        uint256 seedLoopCount = (roundId % providerSafe) + ((roundId + block.number + uint256(finalSeed)) % providerSafe);
        uint256 loopCount = (seedLoopCount % maxLoopCount) + 1;
        _lastSeed = _getHashLoop(loopCount, finalSeed);
    ...
    function _getSeed(int256 answer, uint256 updatedAt, uint80 roundId) private view returns(bytes32){
        return keccak256(
            abi.encodePacked(
                answer, updatedAt, roundId, _lastSeed, msg.sender
            )
        );
    }
    function _getHashLoop(uint256 _count, bytes32 _seed) private view returns(bytes32){
        ...
        for(uint256 i = 0; i < _count; i++){
            bytes32 newSeed = keccak256(abi.encodePacked(i, linkedSeed, _lastSeed, _seed));
            linkedSeed = newSeed;
        }
        return linkedSeed;
    }
